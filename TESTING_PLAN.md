...
#### `src/utils/auth.ts`
- ✅ `saveToken()` - сохранение токена в localStorage
- ✅ `saveRefreshToken()` - сохранение refresh token
- ✅ `removeToken()` - удаление токенов
- ✅ `getToken()` - получение токена
- ✅ `getRefreshToken()` - получение refresh token
- ✅ `transformUserFromAPI()` - преобразование данных пользователя из API
  - Корректная обработка avatarUrls
  - Обработка числового avatar
  - Обработка строкового avatar
  - Формирование URL для аватара
  - Нормализация уровня пользователя
  - Преобразование статистики
  - Обработка null/undefined значений

#### `src/utils/level.ts`
- ✅ `getLevelTitle()` - получение названия уровня
  - Тесты для всех граничных значений (0, 2, 3, 5, 10, 15, 20, 30, 40, 50)
  - Тест для MAX_LEVEL
- ✅ `calculateExperienceToNext()` - расчет опыта для следующего уровня
  - Тесты для разных уровней
  - Проверка формулы
- ✅ `normalizeUserLevel()` - нормализация уровня пользователя
  - Обработка избыточного опыта
  - Повышение уровня при избыточном опыте
  - Ограничение MAX_LEVEL
  - Обнуление опыта при максимальном уровне
...
- ✅ `getErrorMessage()` - извлечение сообщения об ошибке
  ...
  - Обработка RTK Query ошибок
  - Обработка вложенных ошибок
  - Возврат defaultMessage при отсутствии ошибки
- ✅ `isErrorWithData()` - проверка типа ошибки

#### `src/utils/format.ts`
- ✅ `formatDate()` - форматирование даты
  - Корректное форматирование для ru-RU локали
  - Обработка различных форматов дат
- ✅ `formatDateTime()` - форматирование даты и времени
- ✅ `formatShortDate()` - краткое форматирование даты
...
- ✅ `calculateQuestProgress()` - расчет прогресса квеста
  - Пустой массив этапов
  - Один этап
  - Несколько этапов
  - Округление результата
- ✅ `getQuestProgressColor()` - получение цвета прогресса
  - Граничные значения (0, 26, 51, 76, 100)
  - Все возможные цвета
- ✅ `findStageById()` - поиск этапа по ID
  - Существующий этап
  - Несуществующий этап
- ✅ `getActiveStages()` - получение активных этапов
- ✅ `getCompletedStages()` - получение завершенных этапов
- ✅ `getRequirementType()` - определение типа требования
  - Финансовые требования (>= 1000)
  - Волонтеры (< 1000)
- ✅ `transformApiQuestToComponentQuest()` - преобразование квеста из API
  - Обработка всех полей
  - Преобразование steps в stages
  - Обработка requirements
  - Преобразование координат
  - Обработка отсутствующих данных
- ✅ `transformApiQuestsToComponentQuests()` - преобразование массива квестов

#### `src/utils/cityCoordinates.ts`
- ✅ `getCityCoordinates()` - получение координат города
  - Поиск в квестах
  - Поиск в списке городов
  - Возврат null при отсутствии
- ✅ `getOrganizationCoordinates()` - получение координат организации
  - Корректное преобразование строк в числа
  - Обработка различных форматов

#### `src/utils/image.ts`
- ✅ Все функции обработки изображений (если есть)

#### `src/utils/userData.ts`
- ✅ Все функции обработки данных пользователя (если есть)
